...
def construct_borders(length: int, seq_len: int, pred_len: int):
    ...
    border1s = [0, num_train-2000, length - num_test - seq_len, length - seq_len - pred_len]
    # 结束位置
    # border2s = [num_train, num_train + num_vali, length, length]
    border2s = [num_train, num_train+3000, length, length]
    return border1s, border2s


def load_data(args):
    data_type_param = {
        'train': {'shuffle': True, 'drop_last': True, 'batch_size': args.batch_size},
    ...
    for i, csv_file in enumerate(csv_files):
        ...
        if args.target1 not in cols:
            raise ValueError(f"Target1 column '{args.target1}' not found in CSV file '{csv_file}'.")
        if args.target2 not in cols:
            raise ValueError(f"Target2 column '{args.target2}' not found in CSV file '{csv_file}'.")
        cols.remove(args.target1)
        cols.remove(args.target2)
        cols = cols[0:2]
        df_raw = df_raw[cols + [args.target1] + [args.target2]]
